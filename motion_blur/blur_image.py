...
class BlurImage(object): #建立物件(屬性)

    def __init__(self, image_path, PSFs=None, part=None, path__to_save=None): #初始化
        """
        ...
        """
        print(image_path)
        if os.path.isfile(image_path): #看檔案在不在
        ...
        if len(self.result) == 0:
            ...
            if show and save:
                ...
                if self.path_to_save is None:
                    raise Exception('Please create Trajectory instance with path_to_save')
                cv2.imwrite(os.path.join(self.path_to_save, self.image_path.split('\\')[-1]), self.result[0] * 255)
                #os.path.join(self.path_to_save, self.image_path.split('/')[-1])#\反轉譯
                print(self.path_to_save)
                print(self.image_path)
                print(os.path.join(self.path_to_save, self.image_path.split('/')[-1]))
            elif show:
                plt.show()
...
if __name__ == '__main__':
    ...
    for path in os.listdir(folder):
        ...
        BlurImage(os.path.join(folder, path), PSFs=psf,
                  path__to_save=folder_to_save, part=np.random.choice([1, 2, 3])).blur_image(save=True)
